...
5. 각 소제목은 10~25자 이내 (25자 초과 절대 금지! 초과 시 핵심만 남기고 축약할 것)
...
3. 각 소제목은 10~25자 이내 (25자 초과 절대 금지! 초과 시 핵심만 남기고 축약할 것)
...
Output strictly a valid JSON: {"title": "SEO 최적화된 블로그 제목 (메인 키워드로 시작, 15~30자)", "html": "..."}`;
    ...
    async searchPlaceInfo(keyword) {
        const prompt = `"${keyword}" 매장(음식점/카페/가게)의 실제 정보를 검색해줘.
검색 시 "${keyword} 주소 영업시간 메뉴 가격"으로 검색할 것.
네이버 플레이스, 카카오맵, 구글 지도 등 지도 서비스 결과를 우선 참고해.
Output strictly a valid JSON:
...
Output strictly a valid JSON: ["제목1", "제목2", "제목3", "제목4", "제목5"]`;
    ...
    async extractTags(content, excludeHint = '') {
        const excludeRule = excludeHint ? `\n- 다음 태그는 이미 추출됨. 절대 중복하지 말고 새로운 태그만 추출: ${excludeHint}` : '';
        const prompt = `너는 네이버 블로그 SEO 태그 전문가야.
아래 본문을 분석해서 네이버 블로그에 최적화된 태그 ${excludeHint ? '5' : '10'}개를 추출해.

[태그 규칙]
- # 기호 없이 키워드만 반환
- 대중 태그 ${excludeHint ? '2' : '5'}개 (검색량 높은 범용 키워드) + 틈새 태그 ${excludeHint ? '3' : '5'}개 (구체적, 롱테일 키워드) 배합
- 본문의 메인 키워드를 반드시 포함
- 장소명, 브랜드명이 있으면 태그에 포함
- 한국어 태그만 (영어 브랜드명은 예외)
- 2~6글자 길이 권장, 띄어쓰기 없이 붙여쓰기${excludeRule}
...
Output strictly valid HTML only. No JSON wrapping, no explanation.`;
    ...
    async fixSeoIssues(htmlContent, title, mainKeyword, subKeywords, issues, tone = 'friendly') {
        ...
        const RULE_MAP = {
            ...
            intro_short: null, // 동적 생성 (아래에서 metric 기반)
        ...
        const imgMap = [];
        const strippedContent = htmlContent.replace(/<img([^>]*?)src="data:image\/[^"]+"/gi, (match, attrs) => {
            const idx = imgMap.length;
            imgMap.push(match);
            return `<img${attrs}src="__IMG_PLACEHOLDER_${idx}__"`;
        });
