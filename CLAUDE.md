...
## 서비스 정체성
- **이름**: 피클잇 (Piklit) — piklit.vercel.app
...
- **구현 순서**: 요청 이해 → 필요 파일만 확인 → 1~3줄 계획 제시 → 승인 → **즉시 구현 (승인 후 추가 탐색·질문 금지)**
- **세션 시작 시**: `docs/task.md`와 `docs/walkthrough.md`만 확인 (전체 탐색 금지)
- **매 변경 후**: `npm run build`로 빌드 검증 (커밋 시 훅이 자동 실행)
- **커밋 타이밍**: 작업 완료 시 사용자에게 "커밋할까요?" 확인 필수. 임의로 커밋하지 말 것
- **과도한 계획 금지**: 5줄 이상의 계획 설명 불필요. 코드로 보여줄 것
- **반복 읽기 금지**: 같은 파일을 세션 내에서 2회 이상 읽지 않음
- **질문 금지**: 의도가 명확하면 바로 실행. 꼭 필요한 경우만 질문
- **단순함 우선**: 요청한 것만 구현. 1회용 코드에 추상화 금지, 200줄을 50줄로 줄일 수 있으면 줄여라
- **수술적 변경**: 건드려야 할 것만 건드려라. 주변 코드 개선·리팩토링 금지, 기존 스타일 따르기
- **응답 품질**: `docs/ai-guardrail.md` 기준 준수. 위반 의심 시 사용자에게 가드레일 확인을 안내할 것
- **스킬 제안**: 적절한 타이밍에 사용 가능한 스킬을 제안할 것
  - 버그 2개 이상 → `/fix-batch`
  - 작업 완료 → `/pre-commit-qa` → `/commit`
  - 세션 마무리 → `/wrap-up`
  - 배포 전 → `/deploy-check`
