...
### 추가 완료 (2026-03-23 오후)
- [x] 자연스러움 분석 톤별 이모지/비격식 가중치 조정
- [x] 도입부 프롬프트 글자수 반복 강조 4→2회 축소
- [x] 신규 회원가입 디스코드 알림 추가
- [x] CSS font-size 토큰화 전체 완료 (~400건, 10개 파일)
...
- [x] 썸네일 탭 리셋 (2026-03-24 해결 — display:none + BottomSheet 유지)
...
## 완료된 작업 (2026-03-24) — 모바일 UX 버그 수정 4건

### 썸네일 탭 상태 리셋 수정
- [x] MobilePanelContent switch → display:none 방식 (탭 전환 시 상태 유지)
- [x] BottomSheet `return null` → `display:none` (시트 닫힘 시에도 상태 유지)

### 썸네일 UX 개선
- [x] "완료"/"닫기" 클릭 시 자동으로 글 탭 전환 (불필요한 X 제거)
- [x] 아코디언 토글 삭제 — 항상 펼친 상태로 표시

### 모바일 텍스트 잘림 수정
- [x] 소제목 구조 ellipsis → 줄내림 + 글자간격 축소
- [x] 제목 input → textarea 자동 높이 조절
...
- [ ] **자연스러움 적용 시 문단 병합 버그** — 뒷 문단이 따라붙는 문제. 6가지 방식 시도했으나 미해결. 근본 원인 재분석 필요 (2026-03-25 예정)
- [ ] 프롬프트 준수 검수 — AI가 수치 제약(소제목 25자, 80자 문장 등)을 안 따르는 항목 강화 (보고서 작성 완료, 적용 보류)
