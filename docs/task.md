...
#### 우선순위 HIGH
- [ ] 베타 테스터 + 버그 리포트 실제 동작 검증 (콘솔 로그 Safari 호환성 확인)
- [ ] 배포 후 전체 플로우 E2E 테스트 (로그인→위자드→생성→발행)

#### 기능 기획 논의 (다음 세션)
- [ ] AI 전체 리라이트 + 점수 변화 시각화
- [ ] SEO 체크리스트에서 항목별 AI 자동 수정
- [ ] 워너비 스타일 고도화 (게시글 단위 분석 + 스크린샷 영역별 첨부)
- [ ] 휴먼라이징 제안 에디터 인라인 TIP 카드 (원문 클릭 → 본문 형광펜 + 수정안 TIP)
